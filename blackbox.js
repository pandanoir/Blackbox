(function ($, window) {
	$(function () {
		...
		$rform.css3form().on("click", "#save", function (event) {
			...
			$customRadioButton.eq(0).find("a").eq(3).fire().end().end()
				.eq(1).find("a").eq(0).fire().end().end()
				.eq(2).find("a").eq(3).fire().end().end()
				.eq(3).find("a").eq(3).fire().end().end()
				.eq(4).find("a").eq(1).fire();
		});
						...
						for (var i = start.length - 1; i > 0; i -= 1) {
							for (var kk = 0, ll = end.length; kk < ll; kk += 1) {
								if (start[i] < end[kk] && end[kk]) {
									//始め波かっこが終わり波かっこより前にあってendが使用済みでなければ
									var count = start[i] - 2;
									while (count > 0) {
										//セレクタの抽出
										if (str.charAt(count) === "}") {
...
//				console.log(now)
			...
			$("#mes").not(":animated").addClass("on").fadeTo(1500, 0.5, function () {
				$(this).removeClass("on")
			})
		});
		...
		$(window).on("keydown", function (e) {
			e.sp();
			if (e.keyCode === 13 && ((!e.ctrlkey && e.metaKey) || (e.ctrlkey && !e.metakey))) $submit.fire();
		});
